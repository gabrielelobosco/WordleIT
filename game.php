...
<html>
    ...
    <body onload="d()">
        ...
        <?
        $parole = file('5letters.txt', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
...
<script type="text/javascript"> // TASTIERA FISICA
...
<script type="text/javascript"> // TASTIERA SU SCHERMO
...
@keyframes tchangel {
    from {transform: translate(-6px);}
    to {transform: translate(26px);}
}
@keyframes tchanged {
    from {transform: translate(26px);}
...
function ann(res, rows, word) {
  ...
  document.getElementById("second").innerHTML = "PUNTI";
  ...
  if (res == 1) {
    ...
    if (rows == 1)
      document.getElementById("first").innerHTML = "Hai indovinato in <span style='color:green'>" + rows + "</span> tentativo";
    else
      document.getElementById("first").innerHTML = "Hai indovinato in <span style='color:yellow'>" + rows + "</span> tentativi";
  } else {
